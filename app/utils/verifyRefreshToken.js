...
function verifyRefreshToken(refreshtoken) {
  return new Promise((resolve, reject) => {
    JWT.verify(refreshtoken, REFRESH_TOKEN_SECRET_KEY, async (err, payload) => {
      if (err)
        reject(createHttpError.Unauthorized(" وارد1 حساب کاربری خود شوید."));
      ...
      if (!redisRefreshToken)
        reject(createHttpError.Unauthorized("ورود مجدد به حساب کابری نشد."));
      if (refreshtoken === redisRefreshToken) return resolve(mobile);
      reject(
        createHttpError.Unauthorized("ورود مجدد به حساب کابری انجام نشد.")
      );
    });
  });
}
